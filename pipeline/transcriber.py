...
class VibeTranscriber:
    ...
    def _transcribe_one(self, source: Path) -> TranscribeResult:
        ...
        if txt_path.exists():
            return TranscribeResult(
                source=source, success=True, message="txt exists, skipped"
            )
        ...
        if srt_path.exists():
            ...
            return TranscribeResult(
                source=source, success=True, message="srt existed, converted"
            )
        ...
        if proc.returncode != 0:
            return TranscribeResult(
                source=source, success=False, message=proc.stderr.strip() or "failed"
            )
        ...
        if srt_path.exists():
            ...
            return TranscribeResult(
                source=source, success=False, message="srt missing after transcription"
            )
    ...
    def transcribe(self, sources: Iterable[Path]) -> List[TranscribeResult]:
        ...
        worker_count = (
            ...
            else min(32, len(source_list))
        )
        results: List[TranscribeResult] = []
        with ThreadPoolExecutor(max_workers=worker_count) as executor:
            futures = {
                executor.submit(self._transcribe_one, src): src for src in source_list
            }
